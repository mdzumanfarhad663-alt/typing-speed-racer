  ...
  for (const r of results) {
    try {
      ...
      if (existing.length > 0) {
        await db
          .update(schema.rows)
          .set({ resultValue: r.resultValue, timeRange: r.timeRange, source: "scraped", sourceKey: r.sourceKey, updatedAt: now })
        ...
        await db.insert(schema.rows).values({
          ...
          extraLines: [...(r.jodiUrl ? [`jodi_url:${r.jodiUrl}`] : []), ...(r.panelUrl ? [`panel_url:${r.panelUrl}`] : [])],
